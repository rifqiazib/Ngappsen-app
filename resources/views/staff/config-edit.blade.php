    ...
    <div class="">
        ...
        <div class="p-4 bg-neutral border rounded-lg mt-2">
            ...
            <div class="relative overflow-x-auto mt-4">
                <form action="{{ route('staff.configUpdate', ['id' => $staff->id]) }}" method="POST" class="">
                    @csrf
                    @method('PUT')
                    <div class="w-full">
                        ...
                        @foreach($days as $day)
                        <div class="mb-5">
                            <label for="{{ $day }}" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{{ ucfirst($day) }}</label>
                            ...
                            <select id="{{ $day }}" name="id_working_hour[]" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                ...
                                <option value="{{ $hour->id }}"
                                    @if($staffWorkingHours->where('days', $day)->pluck('id_working_hour')->contains($hour->id)) selected @endif>
                                    {{ $hour->working_name }}
                                </option>
                                @endforeach
                            </select>
